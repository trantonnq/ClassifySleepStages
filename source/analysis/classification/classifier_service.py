...
class ClassifierService(object):
    ...
    @staticmethod
    def run_five_class(data_splits, classifier, subject_dictionary, feature_set):
        return ClassifierService.run_in_parallel(ClassifierService.run_single_data_split_five_class,
                                                 data_splits, classifier,
                                                 subject_dictionary, feature_set)
    ...
    @staticmethod
    def run_four_class_with_loaded_model(data_splits, classifier, subject_dictionary, feature_set):
    ...
    @staticmethod
    def run_five_class_with_loaded_model(data_splits, classifier, subject_dictionary, feature_set):

        raw_performances = []
        for ind in range(len(data_splits)):
            data_split = data_splits[ind]
            if ind == 0:
                training_x, training_y = ClassifierInputBuilder.get_five_class_inputs(data_split.training_set,
                                                                                       subject_dictionary=subject_dictionary,
                                                                                       feature_set=feature_set)
                classifier = ClassifierService.train_classifier(training_x, training_y, classifier, 'neg_log_loss')

            testing_x, testing_y = ClassifierInputBuilder.get_five_class_inputs(data_split.testing_set,
                                                                                 subject_dictionary=subject_dictionary,
                                                                                 feature_set=feature_set)
            class_probabilities = classifier.predict_proba(testing_x)
    ...
    def run_in_parallel(function, data_splits, classifier, subject_dictionary, feature_set):
        # run different data splits in parallel
        pool = Pool(cpu_count())
    ...
    @staticmethod
    def run_single_data_split_five_class(data_split, attributed_classifier, subject_dictionary, feature_set):
    ...
    def get_class_weights(training_y):
        ...
        if len(class_weights) == 5:
            class_weight_dict = {0: class_weights[0], 1: class_weights[1], 2: class_weights[2], 3: class_weights[3], 4: class_weights[4]}

        if len(class_weights) == 6:
            class_weight_dict = {0: class_weights[0], 1: class_weights[1], 2: class_weights[2], 3: class_weights[3], 4: class_weights[4], 5: class_weights[5]}
