...
from source.analysis.performance.epoch_performance import ThreeClassPerformance, FiveClassPerformance
...
class CurvePerformanceBuilder(object):
    ...
    def build_roc_from_raw(raw_performances: [RawPerformance], positive_class):
        ...
        # Save to txt file for debugging
        truef = open("truef.txt","a")
        convertf = open("convertf.txt","a")
        truef.write("positive class: ")
        truef.write(str(positive_class)+"\n")
        convertf.write("positive class: ")
        convertf.write(str(positive_class) +"\n")
        ratef = open("roc_rates.txt", "a")
        ...
        for raw_performance in raw_performances:     
            ...
            if (count == 0):
                truef.write("raw_performance true_labels\n")
                for i in raw_performance.true_labels:
                    truef.write(str(i)+ '\n')   
                convertf.write("roc labels\n")
                for i in true_labels:
                    convertf.write(str(i)+ '\n')
            ...
            ratef.write("true positives: " + str(true_positive_rates) +"\n")
            ratef.write("false positives: " + str(false_positive_rates) +"\n")
    ...
    @staticmethod
    def build_five_class_roc_with_binary_search(raw_performances: [RawPerformance]):
        ''' This function runs two binary searches-- first, to match a given wake accuracy value,
        and second, the try to find a threshold that balances the NREM and REM class accuracies '''
        five_class_performances = []

        number_of_wake_scored_as_sleep_bins = 20  # Bin size for the x-dimension of this plot
        false_positive_buffer = 0.001  # How close to the target wake false positive rate we need to be before stopping
        max_attempts_binary_search_wake = 50  # Max number of times the search will try to find a given wake accuracy
        rem_n1_accuracy_tolerance = 1e-2  # How close the NREM and REM class accuracies need to be before stopping
        rem_n2_accuracy_tolerance = 1e-2
        rem_n3_accuracy_tolerance = 1e-2
        max_attempts_binary_search_rem_nrem = 15  # Max times the search will try to balance REM and NREM accuracies
        wake_scored_as_sleep_interpolation_point = 0.4  # Point to report values in the table for
        goal_fraction_wake_scored_as_sleep_spread = []  # Wake false positive rate spread (x-axis of plot)
        ...
        goal_fraction_wake_scored_as_sleep_spread = np.array(goal_fraction_wake_scored_as_sleep_spread)
        cumulative_n1_accuracies = np.zeros(np.shape(goal_fraction_wake_scored_as_sleep_spread))
        cumulative_n2_accuracies = np.zeros(np.shape(goal_fraction_wake_scored_as_sleep_spread))
        cumulative_n3_accuracies = np.zeros(np.shape(goal_fraction_wake_scored_as_sleep_spread))
        cumulative_rem_accuracies = np.zeros(np.shape(goal_fraction_wake_scored_as_sleep_spread))
        cumulative_accuracies = np.zeros(np.shape(goal_fraction_wake_scored_as_sleep_spread))
        ...
        for raw_performance in raw_performances:
            ...
            true_wake_indices = np.where(true_labels == 0)[0]
            true_n1_indices = np.where(true_labels == 1)[0]
            true_n2_indices = np.where(true_labels == 2)[0]
            true_n3_indices = np.where(true_labels == 3)[0]
            true_rem_indices = np.where(true_labels == 4)[0]
            ...
            for goal_fraction_wake_scored_as_sleep in goal_fraction_wake_scored_as_sleep_spread:
                ...
                if binary_search_counter < max_attempts_binary_search_wake:
                    ...
                    sleep_accuracy_spread.append(sleep_accuracy)
                    #nrem_class_accuracies.append(nrem_accuracy)
                    n1_class_accuracies.append(n1_accuracy)
                    n2_class_accuracies.append(n2_accuracy)
                    n3_class_accuracies.append(n3_accuracy)
                    rem_class_accuracies.append(rem_accuracy)
                    accuracies.append(best_accuracy)
                    kappas.append(kappa_at_best_accuracy)
            ...
            sleep_accuracy_spread = np.array(sleep_accuracy_spread)
            #nrem_class_accuracies = np.array(nrem_class_accuracies)
            n1_class_accuracies = np.array(n1_class_accuracies)
            n2_class_accuracies = np.array(n2_class_accuracies)
            n3_class_accuracies = np.array(n3_class_accuracies)
            rem_class_accuracies = np.array(rem_class_accuracies)

            wake_scored_as_sleep_spread = np.insert(wake_scored_as_sleep_spread, 0, 0)
            sleep_accuracy_spread = np.insert(sleep_accuracy_spread, 0, 0)
            #nrem_class_accuracies = np.insert(nrem_class_accuracies, 0, 0)
            n1_class_accuracies = np.insert(n1_class_accuracies, 0, 0)
            n2_class_accuracies = np.insert(n2_class_accuracies, 0, 0)
            n3_class_accuracies = np.insert(n3_class_accuracies, 0, 0)
            rem_class_accuracies = np.insert(rem_class_accuracies, 0, 0)
            ...
            cumulative_n1_accuracies = cumulative_nrem_accuracies + np.interp(
                goal_fraction_wake_scored_as_sleep_spread,
                wake_scored_as_sleep_spread,
                n1_class_accuracies)

            cumulative_n2_accuracies = cumulative_nrem_accuracies + np.interp(
                goal_fraction_wake_scored_as_sleep_spread,
                wake_scored_as_sleep_spread,
                n2_class_accuracies)

            cumulative_n3_accuracies = cumulative_nrem_accuracies + np.interp(
                goal_fraction_wake_scored_as_sleep_spread,
                wake_scored_as_sleep_spread,
                n3_class_accuracies)
            ...
            rem_correct = np.interp(wake_scored_as_sleep_interpolation_point,
                                    wake_scored_as_sleep_spread,
                                    rem_class_accuracies)
            n1_correct = np.interp(wake_scored_as_sleep_interpolation_point,
                                     wake_scored_as_sleep_spread,
                                     n1_class_accuracies)
            n2_correct = np.interp(wake_scored_as_sleep_interpolation_point,
                                     wake_scored_as_sleep_spread,
                                     n2_class_accuracies)
            n3_correct = np.interp(wake_scored_as_sleep_interpolation_point,
                                     wake_scored_as_sleep_spread,
                                     n3_class_accuracies)

            five_class_performance = FiveClassPerformance(accuracy=accuracy,
                                                            wake_correct=1 - wake_scored_as_sleep_interpolation_point,
                                                            ...
                                                            rem_correct=rem_correct,
                                                            kappa=kappa)
            five_class_performances.append(five_class_performance)

        # Average over all the train/test splits
        cumulative_accuracies = cumulative_accuracies / cumulative_counter
        cumulative_n1_accuracies = cumulative_n1_accuracies / cumulative_counter
        cumulative_n2_accuracies = cumulative_n2_accuracies / cumulative_counter
        cumulative_n3_accuracies = cumulative_n3_accuracies / cumulative_counter
        cumulative_rem_accuracies = cumulative_rem_accuracies / cumulative_counter
        ...
        n1_roc_performance = ROCPerformance(false_positive_rates=goal_fraction_wake_scored_as_sleep_spread,
                                              true_positive_rates=cumulative_n1_accuracies)
        n2_roc_performance = ROCPerformance(false_positive_rates=goal_fraction_wake_scored_as_sleep_spread,
                                              true_positive_rates=cumulative_n2_accuracies)
        n3_roc_performance = ROCPerformance(false_positive_rates=goal_fraction_wake_scored_as_sleep_spread,
                                              true_positive_rates=cumulative_n3_accuracies)

        return sleep_wake_roc_performance, rem_roc_performance, nrem_roc_performance, five_class_performances
